...
const notesBackgroundColor =
  JSON.parse(localStorage.getItem("notesbackground")) || [];
...
let descriptionNeededMessage = document.getElementById(
  "description-needed-message",
);
...
function displayNotes() {
  ...
  for (let i = 0; i < notes.length; i++) {
    noteGridHTML += `<div class="note-container-grid"
         ...
         style="background-color:${notesBackgroundColor[i]};overflow:hidden">
         <div class="text-note-container">
         <h1 style="width:100%;white-space:nowrap;overflow-x:auto;scrollbar-width:none;">${notes[i].title}</h1>
    <p style="width:100%;">${notes[i].description}</p></div>
...
function pushToNotes() {
  ...
  if (description && title) {
    ...
    if (
      titleElement.classList.contains("red-outline-input") ||
      descriptionElement.classList.contains("red-outline-input")
    ) {
...
function changeNoteColor() {
  const noteContainerId = document.getElementById(
    `note-container-grid-id-${notes.length}`,
  );
  if (noteContainerId) noteContainerId.style.background = colorPickerValue;
